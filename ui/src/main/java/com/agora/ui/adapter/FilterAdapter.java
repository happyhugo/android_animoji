...
public class FilterAdapter extends RecyclerView.Adapter<FilterAdapter.FilterHolder> {
    ...
    private List<AGFilter> filters;
    protected Context context;
    private int selected = 0;

    public FilterAdapter(Context context, List<AGFilter> filters) {
...
//        holder.thumbImage.setImageResource(thumbResId);
    ...
    public void onBindViewHolder(FilterHolder holder, final int position) {
        int color;
        AGFilter filter = filters.get(position);
    ...
    public interface onFilterChangeListener {
        void onFilterChanged(AGFilter filter);
    }
